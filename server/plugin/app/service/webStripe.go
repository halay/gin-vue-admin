...
type WebStripeSvc struct{ c *client.API }

var WebStripe = new(WebStripeSvc)

func (s *WebStripeSvc) getClient() (*client.API, error) {
	if s.c != nil {
		return s.c, nil
	}
	key := os.Getenv("WEB_STRIPE_SECRET_KEY")
	if key == "" {
		return nil, errors.New("WEB_STRIPE_SECRET_KEY 未配置")
	}
	c := &client.API{}
	c.Init(key, nil)
	s.c = c
...
func (s *WebStripeSvc) EnsureCustomer(email string, userID uint) (string, error) {
	c, err := s.getClient()
	if err != nil {
		return "", err
	}
...
func (s *WebStripeSvc) CreatePaymentIntent(amount int64, currency string, method string, customerID string, metadata map[string]string) (id string, clientSecret string, err error) {
	c, err := s.getClient()
	if err != nil {
		return "", "", err
	}
