...
func (a *PC) GetProductCategoryPublic(c *gin.Context) {
	...
	list, total, err := serviceProductCategory.GetProductCategoryInfoList(ctx, pageInfo, int(*pageInfo.MerchantID))
	if err != nil {
		global.GVA_LOG.Error("获取失败!", zap.Error(err))
		response.FailWithMessage("获取失败:"+err.Error(), c)
		return
...
func (a *PC) GetCategoryTreePublic(c *gin.Context) {
	ctx := c.Request.Context()
	var pageInfo request.ProductCategorySearch
	if err := c.ShouldBindQuery(&pageInfo); err != nil {
		response.FailWithMessage(err.Error(), c)
	...
	tree, err := serviceProductCategory.GetCategoryTree(ctx, int(*pageInfo.MerchantID))
	if err != nil {
		global.GVA_LOG.Error("获取失败!", zap.Error(err))
		response.FailWithMessage("获取失败:"+err.Error(), c)
		return
