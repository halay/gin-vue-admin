  ...
  <el-input v-model.number="searchInfo.balance" placeholder="搜索条件" />
    ...
    <el-select v-model="formData.userId" placeholder="请选择用户" filterable style="width:100%" :clearable="false">
        <el-option v-for="(item,key) in dataSource.userId" :key="key" :label="item.label" :value="item.value" />
             ...
             <el-form-item label="积分余额:" prop="balance">
    <el-input v-model.number="formData.balance" :clearable="false" placeholder="请输入积分余额" />
</el-form-item>
          </el-form>
    </el-drawer>
...
<script setup>
import {
    getUserPointsAccountDataSource,
  createUserPointsAccount,
  deleteUserPointsAccount,
  deleteUserPointsAccountByIds,
  updateUserPointsAccount,
  findUserPointsAccount,
  getUserPointsAccountList
} from '@/plugin/app/api/userPointsAccount'
...
const formData = ref({
  ...
  const getDataSourceFunc = async()=>{
    const res = await getUserPointsAccountDataSource()
    if (res.code === 0) {
...
const getTableData = async() => {
  const table = await getUserPointsAccountList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteUserPointsAccountFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteUserPointsAccountByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateUserPointsAccountFunc = async(row) => {
    const res = await findUserPointsAccount({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteUserPointsAccountFunc = async (row) => {
    const res = await deleteUserPointsAccount({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     ...
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return btnLoading.value = false
              ...
              switch (type.value) {
                case 'create':
                  res = await createUserPointsAccount(formData.value)
                  break
                case 'update':
                  res = await updateUserPointsAccount(formData.value)
                  break
                default:
                  res = await createUserPointsAccount(formData.value)
                  break
