  ...
  <div>
    ...
    <div class="gva-table-box">
      ...
      <el-table ref="multipleTable" style="width: 100%" tooltip-effect="dark" :data="tableData" row-key="id"
        ...
        <el-table-column align="left" label="完成时间" prop="completeAt" width="180">
          <template #default="scope">{{ scope.row.completeAt }}</template>
        </el-table-column>
        <el-table-column align="left" label="用户ID" prop="userId" width="120" />

        <el-table-column align="left" label="任务ID" prop="taskId" width="120" />

        <!-- <el-table-column align="left" label="任务参数" prop="options" width="120" /> -->

        <el-table-column align="left" label="任务类型" prop="type" width="120">
          <template #default="scope">
            {{ filterDict(scope.row.type, ai_task_typeOptions) }}
          </template>
        </el-table-column>
        <el-table-column align="left" label="任务状态" prop="status" width="120">
          <template #default="scope">
            {{ filterDict(scope.row.status, ai_task_statusOptions) }}
          </template>
        </el-table-column>
        <el-table-column align="left" label="备注说明" prop="description" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
          <template #default="scope">
            ...
            <el-button type="primary" link icon="edit" class="table-button"
              @click="updateExtAiTaskFunc(scope.row)">编辑</el-button>
    ...
    <el-drawer destroy-on-close size="800" v-model="dialogFormVisible" :show-close="false" :before-close="closeDialog">
      ...
      <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
        ...
        <el-form-item label="任务类型:" prop="type">
          <el-tree-select v-model="formData.type" placeholder="请选择任务类型" :data="ai_task_typeOptions" style="width:100%"
            filterable :clearable="true" check-strictly></el-tree-select>
        </el-form-item>
        <el-form-item label="任务状态:" prop="status">
          <el-tree-select v-model="formData.status" placeholder="请选择任务状态" :data="ai_task_statusOptions"
...
const getTableData = async () => {
  const table = await getExtAiTaskList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async () => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    const res = await deleteExtAiTaskByIds({ ids })
    if (res.code === 0) {
      ElMessage({
        type: 'success',
        message: '删除成功'
...
// 更新行
const updateExtAiTaskFunc = async (row) => {
  const res = await findExtAiTask({ id: row.id })
  type.value = 'update'
  if (res.code === 0) {
...
// 删除行
const deleteExtAiTaskFunc = async (row) => {
  const res = await deleteExtAiTask({ id: row.id })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
