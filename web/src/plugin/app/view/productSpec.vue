  ...
  <el-input v-model.number="searchInfo.merchantId" placeholder="搜索条件" />
    ...
    <div class="gva-table-box">
        <div class="gva-btn-list">
            ...
            <ExportTemplate  template-id="app_ProductSpec" />
            <ExportExcel  template-id="app_ProductSpec" filterDeleted/>
            <ImportExcel  template-id="app_ProductSpec" @on-success="getTableData" />
        </div>
        <el-table
        ref="multipleTable"
        style="width: 100%"
    ...
    <template #default="scope">
        <span>{{ filterDataSource(dataSource.productId,scope.row.productId) }}</span>
            ...
            <el-table-column sortable align="left" label="规格名" prop="name" width="120" />
    ...
    <el-select v-model="formData.productId" placeholder="请选择商品" filterable style="width:100%" :clearable="false">
        <el-option v-for="(item,key) in dataSource.productId" :key="key" :label="item.label" :value="item.value" />
             ...
             <el-form-item label="规格名:" prop="name">
    <el-input v-model="formData.name" :clearable="true" placeholder="请输入规格名" />
</el-form-item>
             <el-form-item label="排序:" prop="sort">
...
import {
  ...
  findProductSpec,
  getProductSpecList
} from '@/plugin/app/api/productSpec'
...
const formData = ref({
  ...
  const getDataSourceFunc = async()=>{
    const res = await getProductSpecDataSource()
    if (res.code === 0) {
