  ...
  <div>
    ...
    <div class="gva-table-box">
        <div class="gva-btn-list">
            ...
            <ExportTemplate  template-id="app_AgentTransactionDetail" />
            <ExportExcel  template-id="app_AgentTransactionDetail" filterDeleted/>
            <ImportExcel  template-id="app_AgentTransactionDetail" @on-success="getTableData" />
        </div>
        <el-table
        ref="multipleTable"
        style="width: 100%"
        ...
        <el-table-column sortable align="left" label="日期" prop="CreatedAt" width="180">
            ...
            <el-table-column sortable align="left" label="序号" prop="sequenceNumber" width="120" />
            ...
            <el-table-column align="left" label="代理级别" prop="agentLevel" width="120" />
   ...
   <template #default="scope">{{ formatDate(scope.row.transactionTime) }}</template>
            ...
            <el-table-column align="left" label="购买商品" prop="product" width="120" />

            <el-table-column align="left" label="补贴金额" prop="subsidyAmount" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateAgentTransactionDetailFunc(scope.row)">编辑</el-button>
    ...
    <el-input v-model.number="formData.sequenceNumber" :clearable="false" placeholder="请输入序号" />
</el-form-item>
    ...
    <el-select v-model="formData.agentLevel" placeholder="请选择代理级别" style="width:100%" filterable :clearable="false">
    ...
    <el-input-number v-model="formData.subsidyAmount" style="width:100%" :precision="2" :clearable="false" />
</el-form-item>
    ...
    {{ detailForm.sequenceNumber }}
</el-descriptions-item>
                 <el-descriptions-item label="姓名">
    {{ detailForm.name }}
</el-descriptions-item>
                 <el-descriptions-item label="账号ID">
    {{ detailForm.accountId }}
</el-descriptions-item>
                 <el-descriptions-item label="邮箱">
    {{ detailForm.email }}
</el-descriptions-item>
                 <el-descriptions-item label="代理级别">
    {{ detailForm.agentLevel }}
</el-descriptions-item>
                 <el-descriptions-item label="交易时间">
    {{ detailForm.transactionTime }}
</el-descriptions-item>
                 <el-descriptions-item label="购买商品">
    {{ detailForm.product }}
</el-descriptions-item>
                 <el-descriptions-item label="补贴金额">
    {{ detailForm.subsidyAmount }}
</el-descriptions-item>
...
import {
  ...
  deleteAgentTransactionDetail,
  deleteAgentTransactionDetailByIds,
  updateAgentTransactionDetail,
  findAgentTransactionDetail,
  getAgentTransactionDetailList
...
const getTableData = async() => {
  const table = await getAgentTransactionDetailList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteAgentTransactionDetailFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteAgentTransactionDetailByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateAgentTransactionDetailFunc = async(row) => {
    const res = await findAgentTransactionDetail({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteAgentTransactionDetailFunc = async (row) => {
    const res = await deleteAgentTransactionDetail({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
